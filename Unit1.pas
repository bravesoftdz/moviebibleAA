...
var
  ...
  dash : String;
  TAddiction : String;
  bstarthidden, bmemoreadonly : Boolean;
...
begin
    ...
    bstarthidden:= False;
    if bstarthidden then conditionexit:= 96
                    else conditionexit:= 0;
    bmemoreadonly:= True;
...
var i,j,k,m, randchoise, dolzina, spznaci, hiding, plus, pozicijahitm:Integer;
...
begin
  ...
  if (hiding=91) OR (hiding=92) OR (hiding=93) OR (hiding=95) or (hiding=96) then begin
      ...
      for i:=1 to k do begin
        ...
        if hiding = 96 then
          if UTF8Length(Zbor[i]) > 2 then begin
              pozicijahitm:= UTF8Length(Zbor[i]) DIV 2 + 1;
              UTF8Delete(Zbor[i], pozicijahitm,1);
              UTF8Insert(AnsiToUTF8(dash), Zbor[i], pozicijahitm);
              Zbor2[i]:= Zbor[i];
          end
          else Zbor2[i]:= Zbor[i];
...
{    if not(prvpatback) then begin
    ...
    val(arGlava[pozicija], Glavabe);
    str(arKnigaGlava[Knigabe, Glavabe], stKnigaGlavaImaStihovi);
    Form1.Caption:=arKnigaNaslov[Knigabe]+' '+arGlava[pozicija]+','+arStih[pozicija]+'('+stKnigaGlavaImaStihovi+')' + TAddiction;
    end // if not(mbmany)
    else begin
          Memo1.Clear;
          for i:=1 to PreLines do Memo1.Append(' ');
 ...
 {    if not(prvpatback) then begin
  ...
  { if Key=83 then begin  // S for simple reader
   ...
   if Key=82 then               // Memo1 Readonly True/False
    if Memo1.ReadOnly = True then begin
...
begin
  ...
  pauza:=true;
  Form2.Show;
  if bstarthidden then begin
                    Form2.ComboBox6.ItemIndex:= 15;
                    bstarthidden:= False;
             end;
...
begin
    Label1.Left:= round(Form1.Width/2) - 11;
    Label1.Top:= Form1.Height - 29;
    Label3.Left:= round(Form1.Width/2) - 29;
    Label3.Top:= Form1.Height - 29;
    Button1.Left:= round(Form1.Width/2) + 11;
    Button1.Top:= Form1.Height - 29;
